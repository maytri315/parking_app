...
from flask import Blueprint, request, jsonify
from . import db # Assuming db is imported from __init__.py
from .models import User # Assuming User model is imported
...
def register_user():
    ...
    # This check might not be strictly necessary if IntegrityError is handled well,
    # but it can provide a more specific error message upfront.
    ...
    try:
        ...
        new_user.set_password(password) # Calls the set_password method

        db.session.add(new_user)
        db.session.commit()
        return jsonify({"msg": "User registered successfully"}), 201

    except IntegrityError: # Catches database unique constraint violations
        db.session.rollback() # Important to rollback the session on error
        return jsonify({"msg": "Username or email already exists (database conflict)"}), 409 # 409 Conflict
